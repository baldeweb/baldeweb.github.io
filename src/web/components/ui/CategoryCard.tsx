...
export function CategoryCard({ label, icon, selected, onClick, index }: CategoryCardProps) {
  return (
    <button
      ...
      className={[
        "shrink-0 flex flex-col items-center justify-center gap-3 w-[116px] h-[116px] rounded-[2px] border text-center snap-start",
        selected
          ? "border-bs-red bg-bs-card-light text-black shadow"
          : "border-white/10 bg-white/90 text-black hover:border-bs-red",
    ...
    >
      ...
      <span className="text-[0.6rem] font-bold uppercase tracking-[0.14em] leading-tight">
        {label}
      </span>
    </button>
  );
